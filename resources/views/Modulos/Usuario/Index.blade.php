...
@section('title', 'Usuario - Portal DTE')
...
@section('content')
    ...
    @include('Modulos.Usuario.modal.moduloPermision')
    @include('Modulos.Usuario.modal.permisos')
    <div class="card p-1 m-0">
        <div class="card-header p-1">
            ...
            @if(Auth::check() && Auth::user()->categoria == "SuperAdmin")
                <button id="btn-open-modulo" class="btn btn-outline-success btn-sm"><i class="bi bi-shield-lock"></i> Crear módulo <i class="bi bi-folder-plus"></i></button>
            @endif
        </div>
...
@push('scripts')
    ...
    <script src="{{ asset('app/modules/usuario/modulo_permiso.js') }}?v={{ rand() }}"></script>
    <script src="{{ asset('app/modules/asignar_permisos/permiso_usuario.js') }}?v={{ rand() }}"></script>
@endpush
